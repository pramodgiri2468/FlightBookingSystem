        ...
        try (Scanner sc = new Scanner(new File(RESOURCE))) {
            ...
            while (sc.hasNextLine()) {
                ...
                try {
                    ...
                    String noOfSeats = properties[5];
                    Double price = Double.parseDouble(properties[6]); 
                    int maxPassengers = Integer.parseInt(properties[7]);
                   
                    Flight flight = new Flight(id, flightNumber, origin, destination, departureDate);
                    flight.setRemoved(Boolean.parseBoolean(properties[8]));
    ...
    public void storeData(FlightBookingSystem fbs) throws IOException {
        try (PrintWriter out = new PrintWriter(new FileWriter(RESOURCE))) {
            for (Flight flight : fbs.getFlights()) {
                ...
                out.print(flight.getmaxPassengers() + SEPARATOR);
